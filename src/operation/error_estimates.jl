...
function (op::Operator{:Hₑ_up_mix})(ap::T) where T<:AbstractElement
    ΔW²= 0
    W̄² = 0
    Δu²= 0
    ū² = 0
    ...
    𝑤 = ξ.𝑤
        ...
        W̄² += 0.5*(3*p^2/2/K + s₁₁^2/4/G + s₂₂^2/4/G+s₃₃^2/4/G+s₁₂^2/4/G)*𝑤
        Δu² += ((u₁ - ū₁)^2 + (u₂ - ū₂)^2)*𝑤
        ū² += (ū₁^2 + ū₂^2)*𝑤
        Δp² +=(p-p̄)^2
        p̄² +=p̄^2
    end
    return ΔW², W̄², Δu², ū², Δp², p̄²
end
function (op::Operator{:Hₑ_up_mix})(aps::Vector{T}) where T<:AbstractElement
    HₑNorm_ΔW²= 0.0
    HₑNorm_W̄² = 0.0
    L₂Norm_Δu²= 0.0
    L₂Norm_ū² = 0.0
