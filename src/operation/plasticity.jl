...
function (op::Operator{:∫vᵢσdΩ_frictional_contact})(ap::T;k::AbstractMatrix{Float64},fint::AbstractVector{Float64}) where T<:AbstractElement
    ...
    for ξ in 𝓖
        ...
        n₁ = (∂v∂x+tol)/(norm∇v+tol)
        n₂ = (∂v∂y+tol)/(norm∇v+tol)
        s₁ = n₂
        s₂ = -n₁
       # println(666,n₁,666,n₂)
          ...
          Cᵗ₁₂₂₂ = 0.0
            ...
            if εₙ > 0.0  #open
              ...
              if 𝑓 < tol
                  ...
                  σ₂₂ᵗʳ = σ₂₂ + Cᵢᵢᵢᵢ*Δε₂₂ + Cᵢᵢⱼⱼ*Δε₁₁
                  σ₁₂ᵗʳ = σ₁₂ + Cᵢⱼᵢⱼ*Δε₁₂
                  Cᶠ₁₁₁₁ = -μ̄ *sign(τ)*(λ+2.0*μ*n₁*n₁)*2.0*n₁*s₁
                  Cᶠ₂₂₂₂ = -μ̄ *sign(τ)*(λ+2.0*μ*n₂*n₂)*2.0*n₂*s₂
                  Cᶠ₁₁₂₂ = -μ̄ *sign(τ)*(λ+2.0*μ*n₂*n₂)*2.0*n₁*s₁
                  Cᶠ₂₂₁₁ = -μ̄ *sign(τ)*(λ+2.0*μ*n₁*n₁)*2.0*n₂*s₂
                  Cᶠ₁₁₁₂ = -μ̄ *sign(τ)*(2.0*μ*n₁*n₂)*2.0*n₁*s₁
                  ...
                  Cʳ₁₂₂₂ =  μ*(n₂*s₂+n₂*s₂)*(n₁*s₂+n₂*s₁)
                  Cʳ₁₂₁₂ =  μ*(n₁*s₂+n₂*s₁)*(n₁*s₂+n₂*s₁)
                  ξ.σ₁₁ = σ₁₁ᵗʳ + (1-v^2+η)*((Cᶠ₁₁₁₁-Cʳ₁₁₁₁)*Δε₁₁+(Cᶠ₁₁₁₂-Cʳ₁₁₁₂)*Δε₁₂+(Cᶠ₁₁₂₂-Cʳ₁₁₂₂)*Δε₂₂)#+(Cᶠ₁₂₁₁-Cʳ₁₂₁₁)*Δε₁₂+(Cᶠ₂₂₁₁-Cʳ₂₂₁₁)*Δε₂₂)
                  ξ.σ₂₂ = σ₂₂ᵗʳ + (1-v^2+η)*((Cᶠ₂₂₂₂-Cʳ₂₂₂₂)*Δε₂₂+(Cᶠ₂₂₁₂-Cʳ₂₂₁₂)*Δε₁₂+(Cᶠ₂₂₁₁-Cʳ₂₂₁₁)*Δε₁₁)#+(Cᶠ₁₂₂₂-Cʳ₁₂₂₂)*Δε₁₂+(Cᶠ₁₁₂₂-Cʳ₁₁₂₂)*Δε₁₁)
                  ξ.σ₁₂ = σ₁₂ᵗʳ + (1-v^2+η)*((Cᶠ₁₂₁₂-Cʳ₁₂₁₂)*Δε₁₂+(Cᶠ₁₂₁₁-Cʳ₁₂₁₁)*Δε₁₁+(Cᶠ₁₂₂₂-Cʳ₁₂₂₂)*Δε₂₂)#+(Cᶠ₁₁₁₂-Cʳ₁₁₁₂)*Δε₁₁+(Cᶠ₁₂₂₂-Cʳ₁₂₂₂)*Δε₂₂)
...
function (op::Operator{:∫vᵢσdΩ_mc_phasefield})(ap::T;k::AbstractMatrix{Float64},fint::AbstractVector{Float64}) where T<:AbstractElement
    ...
    for  ξ  in  𝓖
        ...
        norm∇v = (∂v∂x^2+∂v∂y^2)^0.5#n = ∇v/norm(∇v)
        nᵢ = (∂v∂x+tol)/(norm∇v+tol)
        nⱼ = (∂v∂y+tol)/(norm∇v+tol)

        # predict phase
        εₙ = ε₁₁*nᵢ*nᵢ + ε₂₂*nⱼ*nⱼ + ε₁₂*nᵢ*nⱼ
        σ₁₁ᵗʳ = σ₁₁ + Cᵢᵢᵢᵢ*Δε₁₁+Cᵢᵢⱼⱼ*Δε₂₂
        σ₂₂ᵗʳ = σ₂₂ + Cᵢᵢᵢᵢ*Δε₂₂+Cᵢᵢⱼⱼ*Δε₁₁
        σ₁₂ᵗʳ = σ₁₂ + Cᵢⱼᵢⱼ*Δε₁₂
        σ₁,σ₂,n₁,n₂ = getσₙ(σ₁₁ᵗʳ,σ₂₂ᵗʳ,σ₁₂ᵗʳ)
            ...
            if εₙ > 0.0
               ...
               Cᵗ₁₂₂₂ = 0.0 
                ...
                if fᵗʳ > tol
                    ...
                    Cᵗ₁₂₁₂=Cᵢⱼᵢⱼ-(1-v^2+η)*2.0(-8.0*cos(𝜙)*cos(𝜙)*μ^2*n₁[1]*n₁[2]*n₂[1]*n₂[2]+4.0*(sin𝜙+1)^2*μ^2*(n₁[1]*n₁[2])^2+4.0*(sin𝜙-1)^2*μ^2*(n₂[1]*n₂[2])^2)/∂fC∂f
