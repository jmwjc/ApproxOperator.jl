...
function (op::Operator{:∫𝒏⋅𝑵⋅𝒗dΓ})(a::T,b::S;k::AbstractMatrix{Float64}) where {T<:AbstractElement,S<:AbstractElement}
    𝓒ᵃ = a.𝓒;𝓖ᵃ = a.𝓖
    𝓒ᵇ = a.𝓒;𝓖ᵇ = a.𝓖
...
function (op::Operator{:∫∇⋅𝑵⋅𝒗dΩ})(a::T,b::S;k::AbstractMatrix{Float64},f::AbstractVector{Float64}) where {T<:AbstractElement,S<:AbstractElement}
    𝓒ᵃ = a.𝓒;𝓖ᵃ = a.𝓖
    𝓒ᵇ = a.𝓒;𝓖ᵇ = a.𝓖
    for (ξᵃ,ξᵇ) in zip(𝓖ᵃ,𝓖ᵇ)
        𝑤 = ξᵃ.𝑤
...
function (op::Operator{:∫∇⋅𝑴⋅𝒏𝒂₃⋅𝒗dΓ})(a::T,b::S;k::AbstractMatrix{Float64},f::AbstractVector{Float64}) where {T<:AbstractElement,S<:AbstractElement}
    𝓒ᵃ = a.𝓒;𝓖ᵃ = a.𝓖
    𝓒ᵇ = a.𝓒;𝓖ᵇ = a.𝓖
    for (ξᵃ,ξᵇ) in zip(𝓖ᵃ,𝓖ᵇ)
        𝑤 = ξᵃ.𝑤
    ...
    for (ξᵃ,ξᵇ) in zip(𝓖ᵃ,𝓖ᵇ)
        ...
        for (i,xᵢ) in 𝓒ᵃ
            ...
            for (j,xⱼ) in 𝓒ᵇ
                J = xⱼ.𝐼
                B̄ₙ = n¹*B̄₁[j] + n²*B̄₂[j]
                k[9*I-8,3*J-2] += N₁₁[i]*B̄ₙ*𝑤
                k[9*I-7,3*J-2] += N₂₂[i]*B̄ₙ*𝑤
                k[9*I-6,3*J-2] += N₁₂[i]*B̄ₙ*𝑤
                k[9*I-5,3*J-1] += N₁₁[i]*B̄ₙ*𝑤
                k[9*I-4,3*J-1] += N₂₂[i]*B̄ₙ*𝑤
                k[9*I-3,3*J-1] += N₁₂[i]*B̄ₙ*𝑤
                k[9*I-2,3*J]   += N₁₁[i]*B̄ₙ*𝑤
                k[9*I-1,3*J]   += N₂₂[i]*B̄ₙ*𝑤
                k[9*I,3*J]     += N₁₂[i]*B̄ₙ*𝑤
            end
            f[9*I-8] += N₁₁[i]*θ*𝑤
            f[9*I-7] += N₂₂[i]*θ*𝑤
...
function (op::Operator{:∫∇⋅𝑴⋅∇𝒂₃⋅𝒗dΓ})(a::T,b::S;k::AbstractMatrix{Float64}) where {T<:AbstractElement,S<:AbstractElement}
    𝓒ᵃ = a.𝓒;𝓖ᵃ = a.𝓖
    𝓒ᵇ = b.𝓒;𝓖ᵇ = b.𝓖
    for (ξᵃ,ξᵇ) in zip(𝓖ᵃ,𝓖ᵇ)
        𝑤 = ξᵃ.𝑤
