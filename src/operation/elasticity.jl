...
function ∫∫σᵢⱼσₖₗdxdy_Taylor(ap::T,k::AbstractMatrix{Float64}) where T<:AbstractElement
    ...
    for ξ in 𝓖
        ...
        for (i,xᵢ) in enumerate(𝓒)
            ...
            for (j,xⱼ) in enumerate(𝓒)
                J = xⱼ.𝐼
                k[3*I-2,3*J-2] += C⁻¹ᵢᵢᵢᵢ*(𝝭[i]*𝝭[j]+∂𝝭∂x[i]*∂𝝭∂x[j]*(xξ-xL)^2+∂𝝭∂y[i]*∂𝝭∂y[j]*(yξ-yL)^2)*𝑤
                k[3*I-2,3*J-1] += C⁻¹ᵢᵢⱼⱼ*(𝝭[i]*𝝭[j]+∂𝝭∂x[i]*∂𝝭∂x[j]*(xξ-xL)^2+∂𝝭∂y[i]*∂𝝭∂y[j]*(yξ-yL)^2)*𝑤
                k[3*I-1,3*J-2] += C⁻¹ᵢᵢⱼⱼ*(𝝭[i]*𝝭[j]+∂𝝭∂x[i]*∂𝝭∂x[j]*(xξ-xL)^2+∂𝝭∂y[i]*∂𝝭∂y[j]*(yξ-yL)^2)*𝑤
                k[3*I-1,3*J-1] += C⁻¹ᵢᵢᵢᵢ*(𝝭[i]*𝝭[j]+∂𝝭∂x[i]*∂𝝭∂x[j]*(xξ-xL)^2+∂𝝭∂y[i]*∂𝝭∂y[j]*(yξ-yL)^2)*𝑤
                k[3*I,3*J]     += C⁻¹ᵢⱼᵢⱼ*(𝝭[i]*𝝭[j]+∂𝝭∂x[i]*∂𝝭∂x[j]*(xξ-xL)^2+∂𝝭∂y[i]*∂𝝭∂y[j]*(yξ-yL)^2)*𝑤
            end
        end
    end
end
    ...
    for ξ in 𝓖
        ...
        for (i,xᵢ) in enumerate(𝓒)
            ...
            for (j,xⱼ) in enumerate(𝓒)
                ...
                # k[3*I-2,3*J-2] += τ*(B₁[i]*B₁[j] + B₂[i]*B₂[j])*𝑤
                # k[3*I-2,3*J]   += τ*(B₁[i]*B₁[j] + B₂[i]*B₂[j])*𝑤
                # k[3*I-1,3*J-1] += τ*(B₁[i]*B₁[j] + B₂[i]*B₂[j])*𝑤
                # k[3*I-1,3*J]   += τ*(B₁[i]*B₁[j] + B₂[i]*B₂[j])*𝑤
                # k[3*I,3*J-2]   += τ*(B₁[i]*B₁[j] + B₂[i]*B₂[j])*𝑤
                # k[3*I,3*J-1]   += τ*(B₁[i]*B₁[j] + B₂[i]*B₂[j])*𝑤
                # k[3*I,3*J]     += τ*(B₁[i]*B₁[j] + B₂[i]*B₂[j])*𝑤
            end
            f[3*I-2] -= τ*(B₁[i]*b₁)*𝑤
            f[3*I-1] -= τ*(B₂[i]*b₂ )*𝑤
    ...
    for ξ in 𝓖
        ...
        for (i,xᵢ) in enumerate(𝓒)
            ...
            for (j,xⱼ) in enumerate(𝓒)
                ...
                k[3*I-2,3*J-2] += (∂𝝭∂x[i]*∂𝝭∂x[j]+∂²𝝭∂x²[i]*∂²𝝭∂x²[j]*(xξ-xL)^2+∂²𝝭∂x∂y[i]*∂²𝝭∂x∂y[j]*(yξ-yL)^2)*𝑤
                k[3*I-2,3*J]   += (∂𝝭∂x[i]*∂𝝭∂y[j]+∂²𝝭∂x²[i]*∂²𝝭∂x∂y[j]*(xξ-xL)^2+∂²𝝭∂x∂y[i]*∂²𝝭∂y²[j]*(yξ-yL)^2)*𝑤
                k[3*I-1,3*J-1] += (∂𝝭∂y[i]*∂𝝭∂y[j]+∂²𝝭∂x∂y[i]*∂²𝝭∂x∂y[j]*(xξ-xL)^2+∂²𝝭∂y²[i]*∂²𝝭∂y²[j]*(yξ-yL)^2)*𝑤
                k[3*I-1,3*J]   += (∂𝝭∂y[i]*∂𝝭∂x[j]+∂²𝝭∂x∂y[i]*∂²𝝭∂x²[j]*(xξ-xL)^2+∂²𝝭∂y²[i]*∂²𝝭∂y²[j]*(yξ-yL)^2)*𝑤
                k[3*I,3*J-2]   += (∂𝝭∂y[i]*∂𝝭∂x[j]+∂²𝝭∂x∂y[i]*∂²𝝭∂x²[j]*(xξ-xL)^2+∂²𝝭∂y²[i]*∂²𝝭∂y²[j]*(yξ-yL)^2)*𝑤
                k[3*I,3*J-1]   += (∂𝝭∂x[i]*∂𝝭∂y[j]+∂²𝝭∂x²[i]*∂²𝝭∂x∂y[j]*(xξ-xL)^2+∂²𝝭∂x∂y[i]*∂²𝝭∂y²[j]*(yξ-yL)^2)*𝑤
                k[3*I,3*J]     += ((∂𝝭∂x[i]*∂𝝭∂x[j]+∂²𝝭∂x²[i]*∂²𝝭∂x²[j]*(xξ-xL)^2+∂²𝝭∂x∂y[i]*∂²𝝭∂x∂y[j]*(yξ-yL)^2)
                                  + (∂𝝭∂y[i]*∂𝝭∂y[j]+∂²𝝭∂x∂y[i]*∂²𝝭∂x∂y[j]*(xξ-xL)^2+∂²𝝭∂y²[i]*∂²𝝭∂y²[j]*(yξ-yL)^2))*𝑤

            end
            f[3*I-2] -= ∂𝝭∂x[i]*b₁*𝑤
            f[3*I-1] -= ∂𝝭∂y[i]*b₂*𝑤
            f[3*I]   -= (∂𝝭∂x[i]*b₂ + ∂𝝭∂y[i]*b₁)*𝑤
        end
    end
end
