    ...
    for ξ in 𝓖
        ...
        for (i,xᵢ) in enumerate(𝓒)
            ...
            for (j,xⱼ) in enumerate(𝓒)
                ...
                k[4*I-5,4*J-4] += N[i]*C⁻¹ᵢᵢⱼⱼ*N[j]*𝑤
                k[4*I-5,4*J-3] += N[i]*C⁻¹ᵢᵢⱼⱼ*N[j]*𝑤

                k[4*I-4,4*J-5] += N[i]*C⁻¹ᵢᵢⱼⱼ*N[j]*𝑤
                k[4*I-4,4*J-4] += N[i]*C⁻¹ᵢᵢᵢᵢ*N[j]*𝑤
                k[4*I-4,4*J-3] += N[i]*C⁻¹ᵢᵢⱼⱼ*N[j]*𝑤

                k[4*I-3,4*J-5] += N[i]*C⁻¹ᵢᵢⱼⱼ*N[j]*𝑤
                k[4*I-3,4*J-4] += N[i]*C⁻¹ᵢᵢⱼⱼ*N[j]*𝑤
                k[4*I-3,4*J-3] += N[i]*C⁻¹ᵢᵢᵢᵢ*N[j]*𝑤

                k[4*I-2,4*J-2] += N[i]*C⁻¹ᵢⱼᵢⱼ*N[j]*𝑤
                k[4*I-1,4*J-1] += N[i]*C⁻¹ᵢⱼᵢⱼ*N[j]*𝑤
    ...
    for (ξₛ,ξᵤ) in zip(𝓖ₛ,𝓖ᵤ)
        ...
        for (i,xᵢ) in enumerate(𝓒ₛ)
            ...
            for (j,xⱼ) in enumerate(𝓒ᵤ)
                ...
                k[6*I-5,3*J-1] -= N[i]*n₃*N̄[j]*𝑤
                k[6*I-5,3*J]   -= N[i]*n₂*N̄[j]*𝑤
