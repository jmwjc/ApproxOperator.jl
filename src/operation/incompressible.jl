    ...
    for ξ in 𝓖
        ...
        for (i,xᵢ) in enumerate(𝓒)
            ...
            for (j,xⱼ) in enumerate(𝓒)
                ...
                k[3*I,3*J]     -= N[i]*N[j]/G*𝑤
            end
        end
    end
end

function (op::Operator{:∫∫sᵢⱼeᵢⱼdxdy})(aᵤ::T,aₛ::S;k::AbstractMatrix{Float64}) where {T<:AbstractElement,S<:AbstractElement}
    𝓒ᵤ = aᵤ.𝓒
    𝓒ₛ = aₛ.𝓒
    𝓖ᵤ = aᵤ.𝓖
    𝓖ₛ = aₛ.𝓖
    for (ξᵤ,ξₛ) in zip(𝓖ᵤ,𝓖ₛ)
        ...
        for (i,xᵢ) in enumerate(𝓒ₛ)
            ...
            for (j,xⱼ) in enumerate(𝓒ᵤ)
                ...
                k[3*I,2*J-1]   += 1/2*N[i]*B₂[j]*𝑤
                k[3*I,2*J]     += 1/2*N[i]*B₁[j]*𝑤
            end
        end
    end
end
...
function (op::Operator{:∫sᵢⱼnⱼgᵢds})(aᵤ::T,aₛ::S;k::AbstractMatrix{Float64},f::AbstractVector{Float64}) where {T<:AbstractElement,S<:AbstractElement}
    ...
    for (ξᵤ,ξₛ) in zip(𝓖ᵤ,𝓖ₛ)
        ...
        for (i,xᵢ) in enumerate(𝓒ₛ)
            ...
            for (j,xⱼ) in enumerate(𝓒ᵤ)
                J = xⱼ.𝐼
                k[3*I-2,2*J-1] += Nₛ[i]*Nᵤ[j]*n₁*n₁₁*𝑤
                k[3*I-2,2*J]   += Nₛ[i]*Nᵤ[j]*n₁*n₁₂*𝑤
                k[3*I-1,2*J-1] += Nₛ[i]*Nᵤ[j]*n₂*n₁₂*𝑤
                k[3*I-1,2*J]   += Nₛ[i]*Nᵤ[j]*n₂*n₂₂*𝑤
                k[3*I,2*J-1]   += Nₛ[i]*Nᵤ[j]*(n₁*n₁₂+n₂*n₁₁)*𝑤
                k[3*I,2*J]     += Nₛ[i]*Nᵤ[j]*(n₁*n₂₂+n₂*n₁₂)*𝑤
            end
            f[3*I-2] += Nₛ[i]*(n₁*n₁₁*g₁+n₁*n₁₂*g₂)*𝑤
            f[3*I-1] += Nₛ[i]*(n₂*n₁₂*g₁+n₂*n₂₂*g₂)*𝑤
            f[3*I]   += Nₛ[i]*(n₁*(n₁₂*g₁+n₂₂*g₂)+n₂*(n₁₁*g₁+n₁₂*g₂))*𝑤
        end
    end
end
